...
def is_mel(name, email):
    ...
    False
    >>> is_mel('Mel Melitpolski', 'mel@google.com')
    True
    >>> is_mel('Mel Sillygoose', 'mel@ubermelon.com')
    True
...
def most_and_least_common_type(treats):
    ...
    #What if the dictionary only has one item in it. 
    >>> treats = [{'type': 'strawberry'}]
    >>> most_and_least_common_type(treats)
    ('strawberry', 'strawberry')

    #What if there is a tie for more or least?
    >>> treats = [{'type':'dessert'},{'type':'appetizer'},{'type':'strawberry'}]
    >>> most_and_least_common_type(treats)
    ('dessert', 'dessert')
